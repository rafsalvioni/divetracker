...
export class DiveMap extends EventTarget
{
    ...
    constructor(svg)
    {
        ...
        var me      = this;
        let size    = Math.round(Math.min(window.innerHeight, window.innerWidth) * 0.9);
        this.size   = size;
        this.mid    = Math.round(size/2);
        this.radius = Math.round(this.mid * 0.9);
    ...
    /**
     * Switchs the current target to the next POI
     * 
     * @returns bool
     */
    switchTarget()
    ...
    async _update()
    {
        ...
        if (this.target) {
            ...
            this.elRoute.setAttribute('transform', 'rotate({0} {1} {2})'.format(Math.round(seg.dir) + rot, this.mid, this.mid));
        }
    }
