...
import { AccelDistanceCounter, PeakResultDistanceCounter, PeakYDistanceCounter } from './dist.js';
import './proto.js';
...
export class ImuProvider extends PostionProvider
{
    ...
    /**
     * Returns default distance counter.
     * 
     * @param {boolean} choose 
     * @returns DistanceCounter
     */
    static distCounter(choose = false)
    {
        const KEY = '_distCounter_';
        let dist  = localStorage[KEY] ?? null;
        if (!dist || choose) {
            dist = prompt("Choose a distance counter\n\n1- Acceleration\n2- Peak Y\n3- Peak 3D", '1');
            localStorage[KEY] = dist;
        }
        ...
        switch (dist.trim()) {
            ...
            case '3':
                return new PeakResultDistanceCounter();
            default:
    ...
    {
        super();
        this.#dist = this.constructor.distCounter();
    ...
    #updatePos()
    {
        ...
        this.#accur += pos.distanceTo(this.#ref) * .06;
        this.#ref   = pos;
