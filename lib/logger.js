...
class TrackLogger extends DataLogger
{
    ...
    logPos(track)
    {
        var mode;
        this._write({
            type: 'pos', trackid: track.id, pos: exportPoint(track.pos),
            newseg: track.provider.mode != mode
        });
        mode = track.provider.mode;
    }
    ...
    export()
    {
        ...
        for (let o of this) {
            if (o.type == 'pos') {
                gpx.addPos(o.pos, o.trackid, o.newseg)
            }
            else if (o.type == 'poi') {
...
class DiveLogger extends DataLogger
{
    ...
    {
        this._write({
            type: 'sample', time: dive.durSec, depth: dive.curDepth
        });
    }
    ...
    logEvent(e, pos = null)
    {
        ...
        if (e.type == 'start') {
            obj = {
                type: e.type, start: e.target.id, pos: exportPoint(pos)
            };
        }
        else if (e.type == 'end') {
            obj = {
            ...
            obj = {
                type:  e.detail, time: e.target.durSec,
                depth: e.target.curDepth,
                mix:   e.target.curMix
            };
        }
        this._write(obj);
    ...
    export()
    {
        ...
        for (let o of this) {
            if (o.type == 'start') {
                uddf.startDive({
                    startDate: new Date(o.start * 1000)
                }, o.pos);
            }
            else if (o.type == 'end') {
                uddf.endDive(o);
...
\t<profiledata>\n";
    ...
    {
        this.#openGroup(dive.startDate);
        let id = dive.startDate.getTime() / 1000;
        if (this.#curDive && id != this.#curDive) {
            this.#closeDive()
        }
        let posStr = '';
        if (pos) {
...
\t\t\t\t<samples>\n`;
    ...
    #openGroup(date)
    {
        let day = parseInt(date.getTime() / 8640000);
        if (this.#curGroup != day) {
            this.#closeGroup();
            this.#xml += `\t\t<repetitiongroup id="rg${day}">\n`;
            this.#curGroup = day;
        }
    }
