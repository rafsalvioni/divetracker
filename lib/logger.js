...
import { SEALEVEL_PRESSURE, dc } from './dc.js';
...
class DiveLogger extends DataLogger
{
    ...
    constructor()
    {
        ...
        var me = this;
        dc.addEventListener('dive', (e) => {
            me.#desatBefore = e.target.desatState;
        });
    }
    ...
    {
        return {
            time: dive.durSec, depth: dive.depth.cur.round(2)
        };
    }
    ...
    set dive(dive)
    {
        ...
        if (dive) { // Is a new valid dive?
            ...
            dive.addEventListener('tankbegin', async (e) => {
                me.#logEvent(e);
            });
            dive.addEventListener('tankend', async (e) => {
                me.#logEvent(e);
            });
    ...
    #logEvent(e)
    {
        ...
        switch (e.type) {
            case 'start':
                let site = e.target.site;
                log = {
                    type: e.type, startDate: e.target.startDate.getTime(),
                    desatBefore: this.#desatBefore, sp: site.env.sp
                };
                if (site) {
                    let pos   = exportPoint(site.pos);
                    pos.fresh = !site.env.sw;
            ...
            case 'end':
                log = {
                    type:       e.type,
                    maxDepth:   e.target.depth.max.round(2),
                    avgDepth:   e.target.depth.avg.round(2),
                    durSec:     e.target.durSec,
                    desatAfter: dc.desatState
                };
                break;
            case 'tankbegin':
                log = {
                    type: e.type,
                    vol: e.detail.vol, start: e.detail.start,
                    mix: Object.enumGetters(e.detail.mix)
                }
                this.#event['gaschange'] = e.detail.mix.id;
                if (!e.target.durSec) { // First sample at depth 0
                    this.logSample(e.target);
                }
                break;
            case 'tankend':
                log = {type: e.type, end: e.detail.end};
                break;
            case 'event':
                this.#event[e.detail] = e.detail;
                break;
            case 'alert':
...
class UddfWriter
{
    ...
    startDive(dive)
    {
        ...
        let desat = dive.desatBefore.desat == 0;
        if (grp == '1' || desat) {
            this.#openGroup(grp);
        }
        this.#closeDive();
        ...
        let xml = {
            ...
            informationbeforedive: {
                ...
                surfacepressure: UddfWriter.#barToPa(dive.sp ?? SEALEVEL_PRESSURE),
        ...
        if (desat) {
            ...
            xml.informationbeforedive.surfaceintervalbeforedive.passedtime = dive.desatBefore.si * 60;
        }
        if (dive.site) {
            xml.informationbeforedive.link = {'@ref': this.#registerSite(dive.site)};
        }
    ...
    addSample(s)
    {
        ...
        const dive = this.#xml.profiledata.repetitiongroup.at(-1).dive.at(-1);

        let xml = {depth: s.depth, divetime: s.time};

        // Gas change event
        if (s.gaschange) {
            xml.switchmix = {'@ref': s.gaschange};
        }
        // Deco added
        if (s.decoadd) {
            let stop = s.decoadd;
        ...
        dive.samples.waypoint.push(xml);
    }
    ...
    end()
    {
        ...
        for (let mix in this.#mixes) {
            this.#xml.gasdefinitions.mix.push({
                '@id': mix, name: mix, o2: this.#mixes[mix].o2, he: this.#mixes[mix].he
            });
        }
    ...
    other(log)
    {
        ...
        if (log.type == 'tankbegin') {
            ...
            dive.tankdata.push({
                '@id': `tank${this.#curDive}/${dive.tankdata.length}`,
                link: {'@ref': id}, tankvolume: (log.vol / 1000),
                tankpressurebegin: UddfWriter.#barToPa(log.start)
            });
        }
        // End tank event
        if (log.type == 'tankend' && dive.tankdata.length) {
            let tank = dive.tankdata.at(-1);
            tank.tankpressureend = UddfWriter.#barToPa(log.end);
        }
    }
        ...
        if (this.#curDive > 0) {
            ...
            if (this.#endInfo) {
                dive.informationafterdive = {
                    ...
                    desaturationtime: this.#endInfo.desatAfter.desat * 60,
                    noflighttime:     this.#endInfo.desatAfter.noFly * 60 
                };
                this.#endInfo = null;
            }
            this.#curDive *= -1;
