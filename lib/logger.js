...
class DiveLogger extends DataLogger
{
    ...
    #flushEvents()
    {
        let events = {};
        if (this.#event) {
            events  = Object.clone({}, this.#event);
        }
        this.#event = {alert: []};
        return events;
    }
    ...
    set dive(dive)
    {
        ...
        if (dive) {
            ...
            dive.addEventListener('start', async (e) => {
                me._write({type: 'site', pos: exportPoint(fus.last.pos), fresh: !conf.dc.salt});
                me.#logEvent(e);
            });
    ...
    addSample(s)
    {
        ...
        if (s.alert && s.alert[0]) {
            for (let alarm of s.alert) {
                alarmstr += `<alarm>${alarm}</alarm>`
            }
        }
        let depth  = Math.rounds(s.depth, 2);
        this.#xml += `\t\t\t\t\t<waypoint><depth>${depth}</depth><divetime>${s.time}</divetime>${mixstr}${alarmstr}</waypoint>\n`;
    }
...
<name>${siteId}</name>\
<geography><latitude>${site.lat}</latitude><longitude>${site.lon}</longitude></geography>\
...
<greatestdepth>${Math.rounds(this.#endInfo.maxDepth, 2)}</greatestdepth>\
<averagedepth>${Math.rounds(this.#endInfo.avgDepth, 2)}</averagedepth>\
