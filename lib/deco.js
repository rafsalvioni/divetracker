...
import { SMAFilter } from "./sfilter.js";
...
// *************** PADI AIR DIVE TABLE *****************
// Padi depths as absolute pressures in bar
...
export class DsatDecoModel extends DecoModel
{
    ...
    constructor(sp = SEALEVEL_PRESSURE)
    {
        super();
        this.#sp = sp;
        this.#filter = new SMAFilter((60000 / conf.track.calcPos).round());
        this.#reset();
    }
    ...
    addChange(absBar, dTime, mix)
    {
        ...
        this.#data.bar   = this.#filter.filter(absBar);
    ...
    ndt()
    {
        return this.#lazyGet('ndt', () => {
            ...
            let i   = DsatDecoModel.getBarIndex(absBar);
            let ndl = 0;
            if (i >= 0) {
