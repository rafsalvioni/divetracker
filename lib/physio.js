...
/**
 * Does a binary search using a function to test.
 * 
 * Returns the nearest value found
 * 
 * fMid receives average value between min and max and should returns:
 * (1)  to inscrease value
 * (-1) to decrease value
 * (0)  to stop
 * 
 * @param {Function} fMid Function to test mid value
 * @param {int} min Min value
 * @param {int} max Max value
 * @returns int
 */
function binTest(fMid, min, max)
{
    let mid;
    while (min <= max) {
        mid = Math.avg(min, max).round();
        switch (fMid(mid)) {
            case 1:
                min = mid + 1;
                break;
...
class BuhlmannDecoModel extends PhysioEffect
{
    ...
    timeLeft()
    {
        return (() => {
            let min = -1;
            let max = Math.ceil(720 / this.#last.bar);

            var me = this;
            return binTest((mid) => { // Binary search
                const clone = me.clone();
                clone.addTime(mid * 60);
                return clone.canAscend ? 1 : -1;
            }, min, max);
    ...
    resetAfter()
    {
        return (() => {
            let min = 0;
            let max = Math.ceil(DAYMIN * this.satur);

            const sf = this.clone();
            const sp = this.#env.sp;
            sf.addChange(sp, null, AIR);

            return binTest((mid) => {
                const clone = sf.clone();
                clone.addTime(mid * 60);
                return clone.ceiling(0) > sp ? 1 : -1;
            }, min, max);
     ...
     * 
     * Gases mixes given will be used according its MOD
     * 
    ...
    stops(...mixes)
    {
        ...
        let ceil    = clone.ceiling(this.#gfLow);
        const stops = [];
        let mi      = 0;
        
        while (!clone.canAscend) {
            const stop  = ceilTo(ceil - sp, this.#phaseB) + sp; // Stop amb pressure, ceil to phase
            const next  = stop - this.#phaseB; // Next stop
            const depth = this.#env.depthAt(stop).round(); // Stop's depth
            const gf    = gfStepAt(this, next); // Max GF in stop
            let mix     = clone.#last.mix; // Current mix

            if (mixes[mi] && mixes[mi].mod >= stop && mixes[mi].mod <= mix.mod) { // Is there a better mix?
                mix = mixes[mi++];
            }
            clone.addChange(stop, null, mix); // Go to stop using current mix

            let sec = 0;
            while (ceil > next && clone.addTime(60)) { // Lets define stop duration
                sec += 60;
                ceil = clone.ceiling(gf);
            }
            
            stops.push({bar: stop, depth, sec, mix, gf});
        }
...
export class BodyState
{
    ...
    nearestEffect()
    {
        let time = 360; let name = '?';
        for (let e in this.#effects) {
            const skipNeg  = ('stops' in this.#effects[e] && this.#effects[e].stops instanceof Function);
