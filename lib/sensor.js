...
class OrientationService extends EventTarget
{
    ...
    #updateOrient(e)
    {
        ...
        let f  = {
            alpha:  e.alpha - conf.imu.compass.offset,
            beta:   e.beta,
            gamma:  e.gamma
        };

        const rotate = screen.orientation.angle;
        // Adjust device natural orientation to screen orientation
        if (rotate) { // 'rotate' is clockwise
            let axis  = {x:f.beta, y:f.gamma};
            let alpha = Angle.use(f.alpha - rotate).pos().deg;
            Coords.rotate(rotate, axis, 'z');
            f = {alpha, beta:axis.x, gamma:axis.y};
        }
    ...
    roundAngle(a)
    {
        return (Math.round(a / conf.imu.compass.scale) * conf.imu.compass.scale) % 360;
    }
    ...
    scaleDiff(radius = 1)
    {
        return Math.toRadians(conf.imu.compass.scale) * radius;
    }
}
...
class MotionService extends EventTarget
{
    ...
    last;
    /**
     * Stores current frequency, in Hz
     * 
     * @type int
     */
    freq;
    ...
    {
        super();
        this.#offset = conf.imu.accelOffset ? conf.imu.accelOffset : {x: 0, y: 0, z: 0};
    ...
    #update(e)
    {
       if (!this.#active(e.acceleration.y != null)) {
        ...
        const rotate = screen.orientation.angle; // Clockwise
        // Adjust accel axis to screen orientation
        Coords.rotate(rotate, f.acceleration, 'z');

        this.last = f;
        this.freq = parseInt(1000 / e.interval);
        this.dispatchEvent(new CustomEvent('change', {
            detail: f
        }));
    }
