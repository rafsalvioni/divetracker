...
export const Coords = {
    ...
    /**
     * Rotate coordinates with angle and axis given.
     *
     * Rotation is not "to" and is anti clockwise.
     *
     * @param {number} theta Angle in Degree, anti clockwise
     * @param {object} coords Coords {x, y, z}
     * @param {string} axis Rotation Axis (x, y, z)
     */
    rotate: (theta, coords, axis) => {
        if ((theta % 360) == 0) {
     ...
     * @param {object} coords Coords {x, y, z}
     * @param {string} order Rotation order, by axis. Lowercase
     */
    rotateByEuler: (ori, coords, order = 'yxz') => {
        const trans = {z: 'alpha', y: 'gamma', x: 'beta'};
        for (let axis of order) {
            if (!(axis in trans)) {
                continue;
            }
            const theta = ori[trans[axis]] ?? 0;
            Coords.rotate(theta, coords, axis);
        }
    },
...
export class Point2D
{
    ...
    bearingTo(p)
    {
        return Coords.bearing(this.x, this.y, p.x, p.y);
    }
...
export class Vector
{
    ...
    resized(size)
    {
        let scale = this.size != 0 ? size / this.size : 0;
    ...
    /**
     * Returns a copy of this vector filtered
     * 
     * If just fX was given, filter will be applied in vector's magnitude.
     * Else, filters will be applied in components separataly.
     * 
     * @param {Filter} fX Filter X / Filter mag
     * @param {Filter} fY Filter Y
     * @param {Filter} fZ Filter Z
     * @returns {Vector}
     */
    filtered(fX, fY = null, fZ = null)
    {
        if (!fY && !fZ) {
            let newsize = fX.filter(this.size);
            return this.resized(newsize);
        }
        else {
            return new Vector(
                fX.filter(this.#x),
                fY ? fY.filter(this.#y) : this.#y,
                fZ ? fZ.filter(this.#z) : this.#z
            );
        }
        return this;
    }
