...
export const Coords = {
    ...
    orientation: (p) => {
        let pitch = Angle.use(Math.atan2(p.x, Math.hypot(p.y, p.z)), true); // Y
        let roll  = Angle.use(Math.atan2(p.y, Math.hypot(p.x, p.z)), true); // X
        let yaw   = Angle.use(Math.atan2(p.z, Math.hypot(p.x, p.y)), true); // Z

        return {
            alpha: yaw.pos().deg,
            beta:  roll.wrap180().deg,
            gamma: pitch.wrap90().deg
        };
    }
};
