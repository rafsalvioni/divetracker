...
// *************** NOOA CNS TABLE *****************
const NOOA_PPO2  = [    .5,  .6,  .7,  .8,  .9, 1.0, 1.1, 1.2, 1.3, 1.4, 1.5, 1.6];
const NOOA_TIMES = [DAYMIN, 720, 570, 450, 360, 300, 240, 210, 180, 150, 120,  45];
// *************** END NOOA CNS TABLE *****************
...
const DiveCalc = {
    ...
    /**
     * Returns max CNS time, in minutes, at given pO2
     * 
     * @param {number} pO2 pO2
     * @returns int
     */
    cnsLimit: (pO2) =>
    {
        for (let i = 0; i < NOOA_PPO2.length; i++) {
            if (pO2 <= NOOA_PPO2[i]) {
                return NOOA_TIMES[i];
            }
        }
...
const DecoCalc = {
     ...
     * @returns number
     */
    etd: (d, mix, env, o2narco = conf.dc.o2narco) =>
    {
        switch (mix.id) {
            case 'air':
...
class GasMix
{
    ...
    {
        return {
            min: Math.max(this.minDepth(env), 0),
    ...
    /**
     * Returns partial pressure of this gas at absolute pressure give
     * 
    ...
    /**
     * Returns CNS percent using this gas, at pA and time given
     * 
...
class DC extends EventTarget
{
    ...
    plan()
    {
        ...
        let res = {
            rg: desat.rg, mix: mix.id, mod: mod, pO2: conf.dc.maxPpo2,
        ...
        while (test) {
            ...
            let rnt  = DecoCalc.rnt(desat.rg, ead);
            ...
            let cnsL = DiveCalc.cnsLimit(pO2);
            let cnsR = parseInt(desat.cns * cnsL);
            let cnsT = cnsL - cnsR;
            time     = Math.min(time, cnsT);
            if (time <= 0) {
                res.break = `CNS >= 100% at ${depth}m`;
                break;
            }

            plan[time] = {depth: depth++, time: time, rnt: rnt, ead: parseInt(ead)};
        }
...
class DesatState
{
    ...
    /**
     * Returns time left to reset CNS
     * 
     * @type int
     */
    get cns()
    {
        ...
        if (isFinite(si) && this.#last.cns > 0) {
            let f = 1 - (si / DiveCalc.cnsTimeLeft(this.#last.cns));
            return Math.max(0, this.#last.cns * f);
        }
        return 0;
    }
...
class Dive extends EventTarget
{
    ...
    {
        let d = this.depth;
        return DecoCalc.etd(avg ? d.avg : d.cur, this.curMix, this.#site.env);
    }
    ...
    #addCns(dt)
    {
        let time   = (dt / 60);
        let pA     = this.pA;
        this.#cns += this.curMix.cns(pA, time);
    }
}
