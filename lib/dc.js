...
export class GasMix
{
    ...
    constructor(o2, he = 0)
    {
        ...
        o2 = (this.o2 * 100).round().intVal();
        he = (this.he * 100).round().intVal();
        switch (true) {
            case o2 == 0:
...
class Tank
{
    ...
    /**
     * Clones current tank using given mix.
     * 
     * @param {GasMix} mix Gas mix
     * @returns {Tank}
     */
    withMix(mix)
    {
        return new Tank(mix, this.vol, this.start);
    }
}
...
class DC extends EventTarget
{
    ...
    #simulateDive(dive, maxDepth)
    {
        ...
        while ((tank = dive.nextTank())) {
            ...
            if (tl.source != 'GAS' || !dive.nextTank()) {
                return tl;
            }
        }
...
class Dive extends EventTarget
{
    ...
    get ascTime()
    {
        ...
        t    += (this.#decoStops.stopsStat().sec / 60);
        return Math.round(t);
    }
...
class DecoStops
{
    ...
    constructor(dive)
    {
        ...
        dive.addEventListener('tankbegin', (e) => {
            ...
            me.#updateStack(e.target); // Update on change tank
        });
    ...
    async #updateStack(dive)
    {
        ...
        this.#stops   = deco.stops(...this.#mixes); // We can redo stops
        this.#i       = 0;
        this.#lastUpd = dive.durSec;
    }

    /**
     * Returns a stops stats
     * 
     * Stats is a object with:
     * - sec: Seconds left to complete all stops
     * - bar: Weighted average stops' ambient pressure
     * 
     * @param {boolean} optional Considers optional stops?
     * @returns {{sec, bar}}
     */
    stopsStat(optional = true)
    {
        let sec  = 0;
        let barw = [];
        for (const stop of this) {
