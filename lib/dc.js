...
const DiveCalc = {
     ...
     * @returns number
     */
    ead: (d, fN2, salt = conf.dc.salt) =>
    {
        let fN = (fN2 / .79);
        return DiveCalc.end(d, fN, salt);
    },
     ...
     * @returns number
     */
    end: (d, fN, salt = conf.dc.salt) =>
    {
        let s = DiveCalc.wf(salt);
        return (d + s) * fN - s;
    },
     ...
     * 
     * @param {number} fN Narcotic (N2 or N2+O2) fraction, in decimal
     * @param {boolean} salt Salt water?
     * @param {number} sp Surface pressure, in bar
     * @returns number
     */
    ...
    /**
     * Returns water factor constants
     * 
     * @param {boolean} salt Salt water?
     * @returns number
     */
    ...
    sp: (alt) =>
    {
        let bar = (1.013 * Math.exp(-alt / 7800.0));
        return bar;
    },
...
const DecoCalc = {
     ...
     * @returns {Array}
     */
    plan: (mix, rg = null, mppO2 = conf.dc.maxPpo2, salt = conf.dc.salt) =>
    {
        let mod = DiveCalc.mod(mix.o2, mppO2, salt).intVal();
        let res = {
            rg: rg, mix: mix.id, mod: mod, pO2: mppO2, water: salt ? 'salt' : 'fresh',
            dives: [],
        };

        let depth = 10; // air depth
        let plan  = {};
        let apox  = DiveCalc.mod(mix.o2, MIN_FO2, salt) > 0;
        while (!apox) {
            let ead  = mix.ead(depth, salt);
...
export function configDive()
{
    try {
        let mixes = [];
        do {
            let o2 = (parseFloat(prompt('Informs mix\'s O2, in percent:', 21)) / 100);
            if (mixes.length && o2 < mixes.at(-1).o2) {
                alert('O2 percent should be greater than previous mix');
                continue;
            }
            let he = (parseFloat(prompt('Informs mix\'s He, in percent:', 0)) / 100);
...
export class GasMix
{
     ...
     * @returns number
     */
    mod(salt = conf.dc.salt, sp = SEALEVEL_PRESSURE)
    {
        return {
            min: DiveCalc.mod(this.#o2, MIN_FO2, salt, sp),
            max: DiveCalc.mod(this.#o2, conf.dc.maxPpo2, salt, sp)
        };
    }
     ...
     * @returns number
     */
    mnd(salt = conf.dc.salt, sp = SEALEVEL_PRESSURE, o2narco = true)
    {
        let fN = o2narco ? 1 - this.#he : this.#n2;
        return DiveCalc.mnd(fN, salt, sp);
    }
        ...
        switch (this.#id) {
            ...
            default:
                return this.#he > 0 && o2narc ?
                    DiveCalc.end(d, 1 - this.#he) :
                    DiveCalc.ead(d, this.#n2);
        }
    }
     ...
     * @returns boolean
     */
    canUseAt(d, salt = conf.dc.salt, sp = SEALEVEL_PRESSURE)
    {
        let od = this.mod(salt, sp);
...
class DC extends EventTarget
{
    ...
    set depth(d)
    {
        ...
        if (!inDive && this.#depth >= 1) {
            ...
            this.#dive = new Dive(conf.dc.salt, this.#pos);
            this.#dive.addEventListener('end', (e) => {
                me.#desat.save(e.target);
            });
            this.dispatchEvent(new CustomEvent('dive', {
                detail: me.#dive
            }));
    ...
    update(track)
    {
        let depth  = -(track.pos.alt - track.first.alt);
        this.depth = depth;
        this.#pos  = track.pos;
        return this;
    }
    ...
    plan()
    {
        return DecoCalc.plan(STARTGAS, this.#desat.rng);
    }
...
class DesatState
{
    ...
    get desat()
    {
        let si = this.si;
        if (isFinite(si)) {
            return Math.max(0, this.#last.desat - si + 1);
        }
        return 0;
    }
    ...
    get noDive()
    {
        ...
        if (isFinite(si) && this.#last.noDive) {
            return Math.max(0, this.#last.noDive - si + 1);
        }
        return 0;
    }
...
class Dive extends EventTarget
{
    ...
    /**
     * 
     * @param {boolean} salt In salt water?
     * @param {GeoPoint} site Divesite
     */
    constructor(salt = conf.dc.salt, site = null)
    {
        super();
        this.#decoStops = new DecoStops(this);
        this.#sp = SEALEVEL_PRESSURE;

        if (site) {
            this.#sp   = DiveCalc.sp(site.alt ?? 0);
            this.#site = site;
    ...
    changeMix(mix)
    {
        if (this.active) {
            ...
            // Add mix
            this.#mixes.push(mix);
            this.#depth.mod = mix.mod(this.#salt, this.#sp);
            this.#depth.mnd = mix.mnd(this.#salt, this.#sp);
    ...
    isMixUsable(mix)
    {
        return mix.canUseAt(this.#depth.cur, this.#salt, this.#sp);
    }
    ...
    /**
     * Equivalent Air/Narcotic Depth
     * 
     * Returned value (EAD or END) depends from current gas mix
     * 
    ...
    get decoState()
    {
        ...
        let state = new DecoState(this.ead(), mixBT, this.#rng[0]);
        return state;
    }
...
class DecoStops
{
    ...
    #update(dive)
    {
        ...
        if (!this.#stops.length && (dive.ead(false) >= 30 || decoState.hasSafetyStop)) { // Safety stop if there's not stop
            this.#addStop(dive, {depth: 5, sec: 3 * 60, required: false});
        }
        if (decoState.emergStop > 0) { // NDL exceeded
            this.#addStop(dive, {depth: 5, sec: decoState.emergStop * 60, required: true});
        }
