...
class Tank
{
    ...
    {
        if (dTime && this.end) {
            const aPA  = Math.avg(this.#last ?? pA, pA);
    ...
    plan()
    {
        ...
        while (test) {
            ...
            try {
                ...
                const pA = dive.pAbs;
                plan[time - asc] = {depth, time, limiter: tl.source, asc, bestmix: GasMix.bestMix(pA).id};
            } catch (e) {
                res.break = e;
                break;
            }
...
class Dive extends EventTarget
{
    ...
    ascSteps(future = 0, optional = false)
    {
        ...
        clone.addSample(this.#depth.cur, future * 60);
    ...
    static *#eachAscStep(dive, optional = false)
    {
        ...
        for (const stop of stops) {
            ...
            dive.addSample(stop.depth); // ascent
            dive.addSample(stop.depth, stop.sec); // stop
            yield;
        }
