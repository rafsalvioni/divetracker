...
const DiveCalc = {
    ...
    {
        let s = DiveCalc.waterFactor(salt);
        return (d + s) * fN - s;
    },
     ...
     * @returns number
     */
    mod: (fO2, mppO2 = conf.dc.maxPpo2, salt = conf.dc.salt) =>
    {
        let s   = DiveCalc.waterFactor(salt);
        let mod = ((mppO2 / fO2) - 1) * s;
        return mod;
    },
     ...
     * @returns number
     */
    mnd: (fN, salt = conf.dc.salt) =>
    {
        return DiveCalc.mod(fN, 4, salt);
    },
...
const DecoCalc = {
    ...
    {
        let mod = parseInt(DiveCalc.mod(mix.o2, mppO2, salt));
        let res = {
...
export class GasMix
{
    ...
    constructor(o2, he = 0)
    {
        ...
        // O2 operational depths, min and max
        this.#mod = [parseInt(DiveCalc.mod(this.#o2, MIN_FO2)), parseInt(DiveCalc.mod(this.#o2))];
        this.#mnd = parseInt(DiveCalc.mnd(1 - this.#he));
     ...
     * @type string
     */
    get mnd() { return this.#mnd; }

    /**
     * Mix's Operational depths {min, max (MOD)}
     * 
     * @type object
     */
    get mod() {
        return {
            min: this.#mod[0], max: this.#mod[1]
        };
    }
    ...
    /**
     * Checks if a current dive can use this mix
     * 
     * @param {number} d Depth, in meters
     * @returns boolean
     */
    canUseIn(dive)
    {
        return this.canUseAt(dive.depth);
    }
}
/**
 * Air mix
    ...
    set depth(d)
    {
        ...
        this.dispatchEvent(new CustomEvent('alert', {
            detail: { // Out of O2 usable boundary!
                type:   'mod',
                active: (!this.curMix.canUseAt(this.#depth.cur))
            }
        }));
        this.dispatchEvent(new CustomEvent('alert', {
            detail: { // MND exceeded!
                type:   'mnd',
                active: (this.#depth.cur > this.curMix.mnd)
            }
        }));
        this.dispatchEvent(new CustomEvent('alert', {
