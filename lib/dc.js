...
class DC extends EventTarget
{
    ...
    plan()
    {
        ...
        while (test) {
            ...
            // NDL Check
            let etd  = DecoCalc.etd(depth, mix, env);
            let rnt  = DecoCalc.rnt(desat.rg, etd);
            let time = DecoCalc.ndl(etd) - rnt;
            if (time <= 0) {
            ...
            let cnsT = CNS.timeLeft(pO2, desat.cns);
            ...
            plan[time] = {depth: depth++, time: time, rnt: rnt, ead: parseInt(etd)};
        }
...
class Dive extends EventTarget
{
    ...
    /**
     * Remainder bottom time, in minutes
     * 
     * We are considering remainder botton time a time to maintain a safe dive
     * 
     * Returns the lowest time of:
     * - No-Deco Time
     * - CNS time to 100%
     * 
     * If negative, "safe time" was exceeded
     * 
     * @type int
     */
    get rbt()
    {
        let pA  = this.pA;
        let cns = CNS.timeLeft(this.curMix.pp(pA, 'o2'), this.#cns);
        return Math.min(this.decoState.ndt, cns);
    }
