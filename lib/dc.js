 ...
 * Maximun pO2 allowed
 */
const MAX_PO2  = conf.dc.maxPpo2 ?? 1.4;
/**
 * O2 percent in Air
 */
const AIR_O2   = .21;
...
class GasMix
{
     ...
     * 
     * A mix is usable when it isnt hypoxic or hyperoxic at depth
     * 
     * @param {number} d Depth, in meters
     * @param {DiveSiteEnv} env DiveSite environment
     * @returns boolean
    ...
    canUseAt(d, env)
    {
        return !(d > this.maxDepth(env) || d < this.minDepth(env));
    }
...
class DC extends EventTarget
{
    ...
    set depth(d)
    {
        ...
        if (!inDive && this.#depth >= 1) {
            ...
            this.#dive.start(FIRSTMIX, this.#desat.state());
            this.#updateDesat();
            inDive = true;
        }
        if (inDive) {
    ...
    plan()
    {
        ...
        let res = {
            rg: desat.rng, mix: mix.id, mod: mod, pO2: MAX_PO2,
        ...
        while (test) {
            ...
            // NDL Check
            let asc  = DiveCalc.tts(depth);
            let etd  = DecoState.etd(depth, mix, env);
            let deco = new DecoState(etd, 0, desat.rng);
            asc     += deco.hasSafetyStop ? 3 : 0;
            let time = deco.ndl;
            if (time <= 0) {
            ...
            plan[time] = {depth: depth++, time: time, rnt: deco.rnt, ead: parseInt(etd)};
        }
...
class DesatState
{
    ...
    get rng()
    {
        let si = this.si;
        if (isFinite(si)) {
            return DecoState.regroup(si, this.#last.group);
        }
        return null;
    }
...
class Dive extends EventTarget
{
     ...
     * @returns self
     */
    start(mix, desatState)
    {
        if (!this.#start) {
            this.#start = parseInt(Date.now() / 1000);
            this.#rng   = [desatState.rng, 0];
    ...
    changeMix(mix)
    {
        if (this.active) {
            ...
            // Add mix
            this.#mixes.push(mix);
            this.#depth.mod = mix.o2Range(this.#site.env);
    ...
    isMixUsable(mix)
    {
        return mix.canUseAt(this.#depth.cur, this.#site.env);
    }
    ...
    get timeLeft()
    {
        let pA    = this.pA;
        let times = {
            cns: CNS.timeLeft(this.curMix.pp(pA, 'o2'), this.#cns),
            ndt: this.decoState.ndt
        }
    ...
    /**
     * Returns the equivalent "table" Dive's depth
     * 
     * @see DecoState.etd
    ...
    {
        let d = this.depth;
        return DecoState.etd(avg ? d.avg : d.cur, this.curMix, this.#site.env);
    }
    ...
    {
        if (this.#depth.cur) {
            return this.pA - this.#site.env.sp;
        }
        return 0;
    }
    ...
    ppMix(mix = null)
    {
        return (mix ?? this.curMix).pp(this.pA);
    }
     ...
     * Adds CNS
     * 
     * @param {int} dt Time in seconds
     */
    #addCns(dt)
    {
        let pA    = this.pA;
        this.#cns += this.curMix.cns(pA, dt);
    }
}
...
class DecoState
{
     ...
     * @param {GasMix} mix Gas mix
     * @param {DiveSiteEnv} env DiveSite environment
     * @param {boolean} o2narco To trimix, considers O2 narcotic?
     * @returns number
     */
    static etd(d, mix, env, o2narco = conf.dc.o2narco)
    {
        switch (mix.id) {
            case 'air':
                return env.depthTo(d, SEAWATER_ENV);
            default:
                let fN  = mix.he > 0 && o2narco ? 1 - mix.he : mix.n2 / AIR_N2;
    ...
    get ndl()
    {
        return this.#lazyGet('ndl', () => {
            let i   = DecoState.getDepthIndex(this.#ead);
            let ndl = 0;
            if (i >= 0) {
                ndl = DIVE_TIMES[i].at(-1);
            }
            return Math.max(ndl - this.rnt, 0);
        });
    }
    ...
    get group()
    {
        return this.#lazyGet('group', () => {
            ...
            if (tbt <= 0) { // No dive and no residual
                return null;
            }
            else if (this.#bt == 0) { // No dive, but there's residual
                return this.#rg;
            }
    
            let di = DecoState.getDepthIndex(this.#ead);
            if (di >= 0) {
    ...
    get hasSafetyStop()
    {
        return this.#lazyGet('safe', () => {
            if (this.ndt < 0) {
                return false;
            }

            let gi = DECO_GROUP.indexOf(this.group);
            if (gi < 0) {
                return false;
            }
    ...
    get rnt()
    {
        return this.#lazyGet('rnt', () => {
            let gi = DECO_GROUP.indexOf(this.#rg);
            if (gi < 0) {
                return 0;
            }
