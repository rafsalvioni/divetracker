...
class DecoState
{
    ...
    /**
     * Return current desaturation time, in minutes
     */
    get desat()
    {
        if (this.#dive) {
            return Math.max(0, DecoCalc.desat(this.#dive.group) - this.si + 1);
        }
        return 0;
    }
...
export class Dive extends EventTarget
{
    ...
    get curMix()
    {
        if (this.#tanks.length) {
            return Object.clone(this.#tanks.at(-1));
        }
        return null;
    }
