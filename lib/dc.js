...
export class GasMix
{
    ...
    /**
     * Calculates Equivalent Air/Narcotic Depth
     * 
     * If mix has He, returns END. Else, EAD
     * 
     * @param {number} d Depth, in meters
     * @param {boolean} salt Salt water?
     * @param {boolean} o2narc Is O2 narcotic?
     * @returns number
     */
    ead(d, salt = conf.dc.salt, o2narc = true)
    {
        let s = salt ? 10 : 10.36;
        let n = this.#he > 0 && o2narc ? (1 - this.#he) : (this.#n2 / .79);
        return (d + s) * n - s;
    }
     ...
     * @returns number
     */
    mod(mppO2 = conf.dc.maxPpo2, salt = conf.dc.salt)
    {
        let mod = ((mppO2 / this.#o2) - 1) * 10 * (salt ? 1 : 1.03);
        return mod;
    }
     ...
     * 
     * @param {boolean} salt Salt water?
     * @param {boolean} o2narc Considers O2 narcotic?
     * @returns number
     */
    mnd(salt = true, o2narc = true)
    {
        let fn  = o2narc ? 1 - this.#he : this.#n2;
...
class DC extends EventTarget
{
    ...
    set depth(d)
    {
        ...
        if (!inDive && this.#depth >= 1) {
            ...
            this.#dive = new Dive(conf.dc.salt);
...
class Dive extends EventTarget
{
    ...
    changeMix(mix)
    {
        if (this.active) {
            ...
            // Add mix
            this.#mixes.push(mix);
            this.#depth.mod = mix.mod(conf.dc.maxPpo2, this.#salt);
            this.#depth.mnd = mix.mnd(this.#salt);
            // Send event...
    ...
    nextMix()
    {
        ...
        if (conf.dc.mixes[i]) {
            ...
            let mod = mix.mod(conf.dc.maxPpo2, this.#salt);
            mix.allowed = mod >= this.#depth.cur;
            return mix;
        }
        return null;
    }
    ...
    set depth(d)
    {
        ...
        this.dispatchEvent(new CustomEvent('alert', {
            detail: { // MOD exceeded!
                type:   'mod',
                active: (this.#depth.cur > this.#depth.mod)
            }
        }));
        this.dispatchEvent(new CustomEvent('alert', {
            detail: { // MND exceeded!
    ...
    ead(avg = true)
    {
        ...
        return this.curMix.ead(d, this.#salt);
    }
...
const DecoCalc = {
    ...
    {
        let mod = parseInt(mix.mod(mppO2, salt));
        let res = {
