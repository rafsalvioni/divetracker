...
export class Dive extends EventTarget
{
     ...
     */
    constructor(o2 = conf.dc.o2, salt = conf.dc.salt)
    {
        super();
        this.#salt  = !!salt;
        this.changeMix(o2);
    ...
    changeMix(o2)
    {
        ...
        // Calc mix's MOD
        mix.mod = parseInt(DiveCalc.mod(mix.o2, conf.dc.maxPpo2, this.#salt));
        // If nitrogen percent changes...
        if (this.#tanks.length && Math.rounds(this.curTank.n - mix.n, 2) != 0) {
            ...
            this.#lastState = lastDive.save(this).state();
        }
        // Add mix
        this.#tanks.push(mix);
    ...
    get decoGroup()
    {
        let ead = this.#ead();
        let dur = this.durMin;
        return DecoCalc.decoGroup(ead, dur, this.#lastState.resGroup);
    }
    ...
    #ndl()
    {
        return DecoCalc.ndl(this.#ead(), this.#lastState.resGroup);
    }
}
...
class DecoStops
{
    ...
    {
        let d    = dive.avgDepth;
        let safe = DecoCalc.safetyStop(d, dive.durMin, dive.lastState.resGroup);
