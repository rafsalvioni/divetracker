...
class DC extends EventTarget
{
    ...
    plan()
    {
        ...
        while (test) {
            ...
            // Simulate dive
            let dt     = TimeCalc.dt(0, depth) * 60;
            const dive = new Dive(site);
            dive.start(tank, this.#si);
            dive.addSample(depth, dt);
            let effect = dive.timeLeft;
            let asc    = dive.ascTime;
            let time   = effect.time - asc;
            dive.end(); // To clear setIntervals
            if (time <= 0) {
                res.break = `${effect.source} reached at ${depth}m`;
                break;
            }
