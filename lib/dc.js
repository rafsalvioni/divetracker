...
export class Dive extends EventTarget
{
    ...
    start(o2 = conf.dc.o2)
    {
        if (!this.#start) {
            ...
            this.#decoBefore = decoInterval.state();
            this.#resGroup   = this.#decoBefore.resGroup;
            this.dispatchEvent(new Event('start'));
            this.changeMix(o2);
            this.dispatchEvent(new CustomEvent('alert', {
                detail: {
                    type:   'nodive',
                    active: this.#decoBefore.noDive > 0
                }
            }));
        }
        return this;
    ...
    end()
    {
        if (this.active) {
            ...
            this.#decoAfter = decoInterval.save(this).state();
        ...
        if (this.active) {
            ...
            if (curMix) {
                if (Math.rounds(curMix.n - mix.n, 2) != 0) { // If nitrogen percent changes...
                    ...
                    this.#resGroup = this.decoState.group;
                }
                // Close current tank
                this.#closeMix();
            }
    ...
    get decoState()
    {
        let state = new DecoState(this.ead(), this.durMin, this.#resGroup);
        return state;
    }
...
const DecoCalc = {
    ...
    plan(rg = null)
    {
        ...
        for (let depth of DIVE_METERS) {
            ...
            let time = DecoCalc.ndl(depth) - rnt;
            if (time > 0) {
