...
export class GasMix
{
    ...
    static from(mix)
    {
        return new GasMix(mix.o2, mix.he ?? 0);
    }
    ...
    /**
     * Returns mix simple object
     */
    get simple() {
        return {
            o2: this.#o2, he: this.#he, n2: this.#n2
        }
    }
    ...
    ead(d)
    {
        switch (this.#id) {
            case 'air':
                return d;
            default:
                return this.#he > 0 ? DiveCalc.end(d, this.#he) : DiveCalc.ead(d, this.#n2);
        }
    }
...
class DC extends EventTarget
{
    ...
    plan()
    {
        return DecoCalc.plan(this.#desat.rng, STARTGAS);
    }
...
const DiveCalc = {
     ...
     * 
     * @param {number} d Depth, in meters
     * @param {number} fHe He fraction, in decimal
     * @param {boolean} salt Salt water?
     * @returns number
     */
    end: (d, fHe, salt = conf.dc.salt) =>
    {
        let s = salt ? 10 : 10.36;
        let n = (1 - fHe);
        return (d + s) * n - s;
    ...
    {
        let mod = ((mppO2 / fO2) - 1) * 10 * (salt ? 1 : 1.03);
        return mod;
    },
...
const DecoCalc = {
     ...
     * @returns {Array}
     */
    plan: (rg = null, mix = AIR, mppO2 = conf.dc.maxPpo2, salt = conf.dc.salt) =>
    {
