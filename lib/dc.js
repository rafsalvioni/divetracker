...
export class GasMix
{
    ...
    constructor(o2, he = 0)
    {
        ...
        this.#fEnd    = (this.he > 0 && o2narco) ? (1 - this.he) : (this.n2 / AIR_N2);

        Object.freeze(this);
    }

    /**
     * Mix's Min Bottom Depth at ambient conditions
     * 
    ...
    mbd(env)
    {
        return Math.max(env.depthAt(this.#hypoP), 0);
    }
    ...
    mod(env)
    {
        return env.depthAt(this.#hyperP);
    }
    ...
    mnd(env)
    {
        return env.depthAt(this.#narcoP);
    }

    /**
     * Calc Equivalent Air Pressure
     * 
    ...
    ead(depth, env)
    {
        ...
        pA     = this.eap(pA);
        return env.depthAt(pA);
    }

    /**
     * Calc Equivalent Narcotic Pressure
     * 
     * For this gas mix and dive pressure, calcs the equivalent pressure which would produce
...
class DC extends EventTarget
{
    ...
    plan()
    {
        ...
        let mod    = mix.mod(env).intVal();
        let noDive = desat.noDive > 0;

        let res = {
            mix: mix.id, mod: mod, pO2: MAX_PO2, rmv: RMV,
...
class DesatState
{
    ...
    save(dive)
    {
        ...
        const decoModel = dive.decoModel;
...
class Dive extends EventTarget
{
    ...
    get timeLeft()
    {
        ...
        let pA    = this.pAbs;
        let times = {
            cns: CNS.timeLeft(tank.mix.pO2(pA), this.cns),
