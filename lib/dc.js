...
export function configDive()
{
    try {
        let mixes = [];
        do {
            let o2 = parseFloat(prompt('Informs mix\'s O2, in percent:', 21));
            let he = parseFloat(prompt('Informs mix\'s He, in percent:', 0));
            mixes.push({o2: o2 / 100, he: he / 100});
...
export class GasMix
{
    ...
    {
        let s = salt ? 10 : 10.36;
        let p = this.#he > 0 && o2narc ? (1 - this.#he) : (this.n2 / .79);
        return (d + s) * p - s;
    }
 ...
 * @type {GasMix}
 */
export const STARTGAS = GasMix.from(conf.dc.mixes[0]);
...
class Dive extends EventTarget
{
     ...
     * 
     * If next gas' MOD < current depth, the MOD will be returned
     * 
     * @returns {GasMix}|int
     */
    nextMix()
