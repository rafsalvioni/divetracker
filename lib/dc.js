...
const DiveCalc = {
     ...
     * 
     * @param {number} d Depth, in meters
     * @param {number} fN2 N2 fraction, in decimal
     * @param {boolean} salt Salt water?
     * @param {number} sp Surface pressure, in bar
     * @returns number
     */
    ead: (d, fN2, salt = conf.dc.salt, sp = SEALEVEL_PRESSURE) =>
    {
        let fN = (fN2 / .79);
        return DiveCalc.end(d, fN, salt, sp);
    },
     ...
     * 
     * @param {number} d Depth, in meters
     * @param {number} fN Narcotic factor, in decimal
     * @param {boolean} salt Salt water?
     * @param {number} sp Surface pressure, in bar
     * @see https://en.wikipedia.org/wiki/Equivalent_air_depth
     * @returns number
     */
    end: (d, fN, salt = conf.dc.salt, sp = SEALEVEL_PRESSURE) =>
    {
        let bar = DiveCalc.swdToBar(d); // Considers given depth at sea
        bar *= fN;
        return DiveCalc.barToDepth(bar, salt, sp); // Considers environment here!
    },

    /**
     * Converts a altitude depth to sea level depth
     * 
     * If sp is not considered "in altitude", do nothing
     * 
     * All altitude depths are considered in fresh water
     * 
     * @param {number} d Depth, in meters
     * @param {number} sp Surface pressure, in bar
     * @returns number
    ...
    esd: (d, sp) =>
    {
        let alt = DiveCalc.inAltitude(sp);
        if (alt) { // If in ~sea level, nothing to do
            let bar = DiveCalc.depthToBar(d, false, sp);
            return DiveCalc.barToSwd(bar);
        }
        return d;
    },

    /**
     * Calculates Max Operational Depth, in meters
     * 
     * @param {number} fO2 O2 fraction, in decimal
     * @param {number} mppO2 Max ppO2 (1.4 - 1.6)
     * @param {boolean} salt Salt water?
     * @param {number} sp Surface pressure, in bar
     * @returns number
     */
    mod: (fO2, mppO2 = conf.dc.maxPpo2, salt = conf.dc.salt, sp = SEALEVEL_PRESSURE) =>
    {
        let bar = (mppO2 / fO2);
        return DiveCalc.barToDepth(bar, salt, sp);
    },

    /**
     * Calculates Max Narcotic Depth, in meters
     * 
     * @param {number} fN Narcotic (.79/N2 or N2+O2) fraction, in decimal
     * @param {boolean} salt Salt water?
     * @param {number} sp Surface pressure, in bar
     * @returns number
     */
    mnd: (fN, salt = conf.dc.salt, sp = SEALEVEL_PRESSURE) =>
    {
        let bar = (4 / fN);
        return DiveCalc.barToDepth(bar, salt, sp);
    },
    ...
    swf: (salt) =>
    {
        ...
        return salt ? .101043 : .0981;
    },

    /**
     * Calculates surface pressure at given altitude, in bar
     * 
     * @param {number} alt Altitude, in meters
     * @see https://github.com/subsurface/subsurface/blob/b56b7abcf5d6f47f86493f7f246697bd7a785b79/core/units.h#L270
     * @returns number
     */
    sp: (alt) =>
    {
        let bar = (SEALEVEL_PRESSURE * Math.exp(-alt / 7800.0));
        return bar;
    },

    /**
     * Checks if a surface pressure is considered in altitude
     * 
     * Are considered altitude dives above 1000ft / 304m / .97bar
     * 
    ...
    inAltitude: (sp) =>
    {
        return sp <= .9262; // Bar at ~700m / 2300ft
    },

    /**
     * Converts a pressure to depth, in meters
     * 
     ...
     * @returns number
     */
    barToDepth: (bar, salt = conf.dc.salt, sp = SEALEVEL_PRESSURE) =>
    {
        let sw = DiveCalc.swf(salt);
        let d  = (bar - sp) / sw;
        return d;
    },

    /**
     * Converts a depth to pressure in bars
     * 
     * @param {number} d Depth, in meters
     * @param {boolean} salt Salt water?
     * @param {number} sp Surface pressure, in bar
     * @returns number
     */
    depthToBar: (d, salt = conf.dc.salt, sp = SEALEVEL_PRESSURE) =>
    {
        // Pt = P0 + d * g * h
        let sw  = DiveCalc.swf(salt);
        let bar = sw * d + sp;
    ...
    /**
     * Converts a sea level depth to bar
     * 
     * @param {number} d Depth, in meters
     * @returns number
     */
    swdToBar: (d) =>
    {
        return DiveCalc.depthToBar(d, true, SEALEVEL_PRESSURE);
    },

    /**
     * Converts a pressure to sea level depth
     * 
     * @param {number} bar Pressure in bar
     * @returns number
     */
    barToSwd: (bar) =>
    {
        return DiveCalc.barToDepth(bar, true, SEALEVEL_PRESSURE);
    }
}
// *************** PADI AIR DIVE TABLE *****************
...
const DecoCalc = {
     ...
     * DOESN'T USE RNT!!!
     * 
     * @param {number} d Depth in meters at sea level
     * @returns int
     */
    ndl: (d) =>
    {
    ...
    /**
     * Returns a list of possible nodeco dives considering a eventual residual group
     * 
     * Each item is a object {d (depth), t (time)}, where time is adjusted using RNT
     * 
     * @param {GasMix} mix Gas mix
     * @param {string} rg Residual group
     * @param {number} mppO2 Max ppO2
     * @param {DiveSiteEnv} env Dive site environment
     * @returns {Array}
     */
    plan: (mix, rg = null, mppO2 = conf.dc.maxPpo2, env = DEFAULT_ENV) =>
    {
        let mod = DiveCalc.mod(mix.o2, mppO2, env.salt, env.sp).round();
        let res = {
            rg: rg, mix: mix.id, mod: mod, pO2: mppO2,
            water: env.salt ? 'salt' : 'fresh', sp: env.sp,
            dives: [],
        };

        let depth = 10; // air depth
        let plan  = {};
        let apox  = DiveCalc.mod(mix.o2, MIN_FO2, env.salt, env.sp) > 0;
        while (!apox) {
            let ead  = mix.ead(depth, true, env);
...
export class GasMix
{
    ...
    /**
     * Mix's Operational depths {min, max (MOD)} at ambient conditions
     * 
     * @param {DiveSiteEnv} env Site environment
     * @returns number
     */
    mod(env)
    {
        return {
            min: DiveCalc.mod(this.o2, MIN_FO2, env.salt, env.sp),
            max: DiveCalc.mod(this.o2, conf.dc.maxPpo2, env.salt, env.sp)
        };
    }

    /**
     * Mix's Narcotic depth at ambient conditions
     * 
     * @param {DiveSiteEnv} env Site environment
    ...
    {
        let fN = o2narco ? 1 - this.he : .79 / this.n2;
        return DiveCalc.mnd(fN, env.salt, env.sp);
    }

    /**
     * Returns mix's EAD/END.
     * 
     * If mix has helium, returns END. Else, EAD
     * 
     * It considers environment too, using surface pressure and water type
     * 
     * @param {number} d Depth, in meters
     * @param {boolean} o2narc To END, considers O2 narcotic?
     * @param {DiveSiteEnv} env Site environment
     * @returns number
     */
    ead(d, o2narc = true, env = DEFAULT_ENV)
    {
        switch (this.id) {
            case 'air':
                return DiveCalc.esd(d, env.sp);
            default:
                return this.he > 0 && o2narc ?
                    DiveCalc.end(d, 1 - this.he, env.salt, env.sp) :
                    DiveCalc.ead(d, this.n2, env.salt, env.sp);
        }
    }
 ...
 * @type {GasMix}
 */
export const AIR = new GasMix(.21);
Object.freeze(AIR);
 ...
 * @type {GasMix}
 */
export const STARTGAS = conf.dc.mixes.length ? GasMix.from(conf.dc.mixes[0]) : AIR;
Object.freeze(STARTGAS);
...
class DiveSite
{
    ...
    constructor(point)
    {
        this.pos = point;
        let alt  = point.alt ?? 0;
        let sp   = DiveCalc.sp(alt);
        this.env = new DiveSiteEnv(sp);
        Object.freeze(this);
    }
}

/**
 * Represents a site environment conditions
 */
class DiveSiteEnv
{
    /**
    ...
    /**
     * Default sea level environment
     * 
     * @returns {DiveSiteEnv}
     */
    static default()
    {
        return new DiveSiteEnv();
    }
    ...
    constructor(sp = SEALEVEL_PRESSURE)
    {
        ...
        this.salt = DiveCalc.inAltitude(sp) ? false : conf.dc.salt;
        Object.freeze(this);
    }

    /**
     * Returns total pressure, in bar, at given depth considering
     * environment
     * 
     * @param {number} d Depth, in meters
     * @returns number
    ...
    pressureAt(d)
    {
        return DiveCalc.depthToBar(d, this.salt, this.sp);
    }
}
const DEFAULT_ENV = DiveSiteEnv.default();
...
class DC extends EventTarget
{
    ...
    set depth(d)
    {
        ...
        if (!inDive && this.#depth >= 1) {
            ...
            this.#dive.start(STARTGAS, this.#desat.rng);
            this.#updateDesat();
            inDive = true;
        }
        if (inDive) {
    ...
    /**
     * Returns a list of possible no-deco dives using current RNG and DC settings
     * 
    ...
    plan()
    {
        ...
        return DecoCalc.plan(STARTGAS, this.#desat.rng, conf.dc.maxPpo2, env);
    }
...
class Dive extends EventTarget
{
    ...
    /**
     * 
     * @param {DiveSite} site Site environment
     */
    constructor(site)
    {
        super();
    ...
    changeMix(mix)
    {
        if (this.active) {
            ...
            // Add mix
            this.#mixes.push(mix);
            this.#depth.mod = mix.mod(this.#site.env);
    ...
    /**
     * Returns next not used Gas mix defined, or null
     * 
     * @returns {GasMix}
     */
    nextMix()
    ...
    set depth(d)
    {
        ...
        this.dispatchEvent(new CustomEvent('alert', {
            detail: { // Out of O2 usable boundary!
                type:   'mod',
    ...
    etd(avg = true)
    {
        ...
        return this.curMix.ead(d, true, this.#site.env);
    }
...
class DecoStops
{
    ...
    constructor(dive)
    {
        ...
        this.#stopDepth = DiveCalc.esd(5, dive.site.env.sp).round(0).intVal();
    }
