...
class DC extends EventTarget
{
    ...
    set depth(d)
    {
        ...
        if (!inDive && this.#depth >= 1) {
            ...
            let sp   = SEALEVEL_PRESSURE;
            let salt = conf.dc.salt;

            if (gps.active) {
                let alt = gps.last.pos.alt;
                sp = DiveCalc.sp(gps.last.pos.alt);
                if (alt >= 1000) {
                    salt = false;
                }
            }

            this.#dive = new Dive(salt, sp);
...
class Dive extends EventTarget
{
     ...
     * @param {number} sp Surface pressure, in bar
     */
    constructor(salt = conf.dc.salt, sp = SEALEVEL_PRESSURE)
    {
        super();
        this.#decoStops = new DecoStops(this);
        this.#salt = !!salt;
        this.#sp = parseFloat(sp);
    }
