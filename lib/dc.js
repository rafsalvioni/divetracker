...
class DC extends EventTarget
{
    ...
    set depth(d)
    {
        ...
        if (!inDive && this.#depth >= 1) {
            ...
            this.#dive.start(conf.dc.o2, this.#desat.rng);
            this.#updateDesat();
            inDive = true;
        }
        if (inDive) {
    ...
    plan()
    {
        return DecoCalc.plan(
            this.#desat.rng, conf.dc.o2, conf.dc.maxPpo2, conf.dc.salt
        );
    }
...
class Dive extends EventTarget
{
    ...
    changeMix(o2)
    {
        if (this.active) {
            ...
            if (curMix) {
                if (Math.rounds(curMix.n2 - mix.n2, 2) != 0) { // If nitrogen percent changes...
    ...
    set depth(d)
    {
        ...
        if (this.#depth.last) {
            ...
            this.#depth.avg = ((this.#depth.avg * this.#depth.last) + (this.#depth.cur * dt)) / dur;
        }
        else {
    ...
    ead(avg = true)
    {
        ...
        return DiveCalc.ead(d, this.curMix.n2, this.#salt);
    }
