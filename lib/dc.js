...
class DC extends EventTarget
{
    ...
    set depth(d)
    {
        ...
        if (!inDive && this.#depth >= 1) {
            ...
            this.#dive.start(FIRSTMIX, this.#desat);
            this.#updateDesat();
            inDive = true;
        }
        if (inDive) {
...
class Dive extends EventTarget
{
    ...
    set depth(d)
    {
        ...
        if (this.#depth.last) {
            ...
            this.#decoModel.addChange(pAbs, dtSec, curMix);
            this.#cns += curMix.cns(pAbs, dtSec);
        }
        else {
    ...
    ppMix(mix = null)
    {
        return (mix ?? this.curMix).pp(this.pAbs);
    }
...
class DecoStops
{
    ...
    async #update(dive)
    {
        ...
        for (let stop of decoModel.stops()) {
            stop.depth = dive.site.env.depthAt(stop.bar);
