...
class DC extends EventTarget
{
    ...
    plan()
    {
        ...
        let noDive = this.#si.noDive > 0;
        const body = this.#si.bodyState(env.sp);
        ...
        while (test) {
            ...
            // Body check
            let bodyAt = body.test(pA, mix);
            let effect = bodyAt.nearestEffect();
            let asc    = DiveCalc.tts(depth);
            asc       += bodyAt.decoModel.stopTime(true);
            let time   = effect.time - asc;
            if (time <= 0) {
                res.break = `${effect.name.toUpperCase()} effect reached at ${depth}m`;
                break;
            }
            // Tanks Test
            ...
            let ead = env.depthAt(mix.ead(pA)).round().intVal();
            let end = env.depthAt(mix.end(pA)).round().intVal();
            asc     = asc.round().intVal();
            plan[time] = {depth, time, ead, end, asc, bestmix: GasMix.bestMix(pA).id};
...
class SurfaceInterval
{
     ...
     * @type {BodyState}
     */
    bodyState(sp = SEALEVEL_PRESSURE)
    {
        let si = this.si;
        const body = new BodyState(sp);
        if (isFinite(si)) {
...
class Dive extends EventTarget
{
     ...
     * @param {SurfaceInterval} desatState Desat state
     * @returns self
     */
    start(tank, desatState)
    {
        if (!this.#start) {
            ...
            this.#bodyState = desatState.bodyState(this.#site.env.sp);
    ...
    set depth(d)
    {
        ...
        // Updates body state
        this.#bodyState.addChange(pAbs, dtSec, curTank.mix);
        // Uses current tank
...
class DecoStops
{
    ...
    async #update(dive)
    {
        ...
        let decoModel = dive.bodyState.decoModel;

        for (let stop of decoModel.stops()) {
            stop.depth = dive.site.env.depthAt(stop.bar).round().intVal();
    ...
    #addStop(dive, s)
    {
        ...
        {
            if (!s1.init) {
                return (s1.required ^ s2.required) && s2.required;
            }
            return false;
        }
