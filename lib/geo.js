...
class Track extends EventTarget
{
    ...
    /**
     * Adds the current position to the POI list.
     * 
     */
    registerPoi()
    {
        let pos = this.pos;
        this.poi.push(pos);
        this.dispatchEvent(new TrackEvent('poi', pos));
    }
    ...
    /**
     * Return a segment to go from current position to a POI indicated by index given.
     * 
     * @param {int} poiIdx 
     * @returns Segment
     */
    toPOI(poiIdx)
    {
        if (poiIdx >= 0 && poiIdx < this.poi.length) {
            return this.toPoint(this.poi[poiIdx]);
        }
        throw 'Invalid POI index';
    }
